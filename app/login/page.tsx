  ...
  const handleSubmit = async () => {
    ...
    if (mode === "reset") {
      ...
      if (error) {
        ...
        setMessage("パスワードリセット用のメールを送りました。メールを確認してください。");
      }
      setLoading(false);
      return;
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {mode === "reset" ? (
          <>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8, color: "#1a1a1a" }}>パスワードをお忘れの方</div>
            <div style={{ fontSize: 13, color: "#888", marginBottom: 20 }}>登録済みのメールアドレスを入力してください。リセット用のリンクをお送りします。</div>
            <input
              type="email"
              placeholder="メールアドレス"
              value={email}
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
              {[
                ...
                <button key={opt.val} onClick={() => switchMode(opt.val as "login" | "signup")} style={{
            ...
            {mode === "login" && (
              <div style={{ textAlign: "right", marginTop: 8 }}>
                <button onClick={() => switchMode("reset")} style={{
                  ...
                  fontSize: 12, color: "#5a8a5a", textDecoration: "underline",
                }}>
                  パスワードを忘れた方はこちら
                </button>
              </div>
        ...
        <button onClick={handleSubmit} disabled={loading} style={{
          ...
          {loading ? "処理中..." : mode === "reset" ? "リセットメールを送る" : mode === "login" ? "ログイン" : "アカウントを作成"}
        </button>

        {mode === "reset" && (
          <button onClick={() => switchMode("login")} style={{
            width: "100%", marginTop: 10, padding: "12px",
            background: "none", border: "1px solid #e8e2d8",
